...
(function () {
  ...
  function initOne(root, opts = {}) {
    ...
    function drawLedger(){
      ...
      // 1) Friction: 0 → Wfric
      fillBetween(s => s.Wfric, s => 0, "rgba(244,119,74,0.30)");
      // 2) Inelastic: Wfric → Wfric+Winel
      fillBetween(s => s.Wfric + s.Winel, s => s.Wfric, "rgba(231,188,79,0.30)");
      // 3) Motion: (Wfric+Winel) → (E0)  [top is constant = total energy]
      fillBetween(s => state.E0, s => s.Wfric + s.Winel, "rgba(80,170,155,0.35)");
